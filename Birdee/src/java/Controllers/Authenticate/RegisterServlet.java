...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        user.setPassword(encode.EncoderMD5(request.getParameter("pwd")));
        user.setAddress(request.getParameter("address"));
        Date dob = Date.valueOf(request.getParameter("dob"));
        user.setDob(dob);

        boolean isExist = uDao.isUserExist(user.getUsername());
        if (isExist) {
            request.getSession().setAttribute("isFail", true);
            request.getSession().setAttribute("msg", "Tên đăng nhập  đã được đăng ký, vui lòng thử lại!");
